...
// }
 ...
 class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    this.querySelectorAll(".has-submenu details > summary").forEach(
      (summary) => {
        summary.addEventListener("click", this.onSummaryClick.bind(this));
      }
    );
  ...
  onSummaryClick(event) {
    ...
    const panel = details.querySelector(".menu-drawer__submenu");

    if (!panel) return;

    if (details.open) {
      this.close(details, panel, summary);
    } else {
      this.open(details, panel, summary);
    }
  }

  open(details, panel, summary) {
    details.open = true;
